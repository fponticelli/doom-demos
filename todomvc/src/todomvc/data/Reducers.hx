...
class Reducers {
  ...
  public static function todos(state: ReadonlyArray<TodoItem>, action: TodoAction): ReadonlyArray<TodoItem> return switch action {
    case Add(text):
      state
    ...
    case Toggle(id):
      ...
      state.slice(0, index)
        .concat([state[index].shallowMerge({ completed: !state[index].completed })])
    ...
    case UpdateText(id, text):
      var index = getIndex(state, id);
      state.slice(0, index)
        .concat([state[index].shallowMerge({ text: text })])
        .concat(state.slice(index + 1));
    case ClearCompleted:
